...
class MetaCorgiSnacks
  ...
  #   food = name.to_s
  #   if treats.include?(food)
  #     m_piece = "get_#{food}_"
  #     info = @snack_box.send((m_piece + "info"), @box_id)
  #     tastiness = @snack_box.send((m_piece + "tastiness"), @box_id)
  #     result = "#{food[0].upcase + food[1..-1].downcase}: #{info}: #{tastiness} "
  #     tastiness > 30 ? "* #{result}" : result
  #   else
  #     super
  #   end#else just raise standard error.
  # end
